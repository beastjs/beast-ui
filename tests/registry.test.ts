...
describe('published CLI', () => {
  test('packs without runtime dependencies and installs components under Node', async () => {
    ...
    const manifest = JSON.parse(await readFile(path.join(cwd, 'node_modules/@beastjs/cli/package.json'), 'utf8'))
    expect(manifest.dependencies).toBeUndefined()
    expect(manifest.license).toBe('MIT')
    expect(await readFile(path.join(cwd, 'node_modules/@beastjs/cli/LICENSE'), 'utf8')).toBe(await readFile(path.join(root, 'LICENSE'), 'utf8'))
